...
// 
// Created by Daniel Kaminski bases on the DirectX exmples from
// Microsoft Corporation.
//--------------------------------------------------------------------------------------
#define SHADOW_EPSILON 0.025900f//0.0285f more dark shadows <-----o------> less shadows
                         ...
                         uniform bool bAnimate )
{
	...
	sunPosition =		mul(LightPosition, (float3x3) g_mRotate); //sun position
	ObserverPosition =	mul(ObsPositionV, (float3x3) g_mRotate); //for small light at the observer position

	worldPosition =		mul(input.vPos, g_mWorldRotate ); //objeect position
	Output.viewDirection =  normalize( worldPosition - (float3) ObsPosition );
